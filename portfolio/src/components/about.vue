<template>
    <div id="about-section" class="app">
        <div class="container box">
            <h2 class="content1">About Me</h2>
            <p class="content2">
                I am an undergraduate at Singapore Management University, pursuing a Bachelor's Degree in Information Systems. I am taking a dual track in Digitalisation & Cloud Solutions, and Smart-City Management & Technology.
            </p>
            <br>
            <p class="content2">
                I enjoy meeting new people and hearing their stories, as I believe everyone holds a unique perspective. Working together in a team allows us to approach the same problem from multiple angles. 
                Currently seeking full-time employment in Product Management or Software Engineering roles – I'm excited to leverage technology to drive meaningful change. Feel free to reach out! 
            </p>
        </div>
    </div>
</template>

<script></script>

<style scoped>
    .box {
        border: 1px solid rgb(93, 135, 188);
        padding: 10px;
        border-radius: 12px;
        margin-left: auto;
        margin-top: 200px;
        margin-bottom: 50px;
        width: 60%;
    }

    @media (max-width: 767px) {
        .box {
            margin-top: 50px;
        }
    }

</style>